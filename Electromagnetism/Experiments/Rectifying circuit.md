...
The point of the circuit is to turn a a fixed-frequency potential[^1] into a constant one (basically an AC to DC transformer). The oscillation period is given by a
$$\tau=R^{*}C$$
...
The current iteration of the circuit is very inefficient, as the diode is wasting at least half of the energy that we put in. A more efficient method is to use a [[diode bridge]] in a [[transformer]].

[^1]: For reference, the power grid (AC) runs at 50 Hz in Europe and 60 Hz in the US.
