...
The experiment employs a [[Braun tube]], which is the first historical form of the [[cathode ray tube]]. It's a vacuumed glass tube with an electron blaster on the smaller end, aimed straight through the tube. Not far after the blaster, a set of metallic plates creates a parallel plate [[capacitor]] with a controllable electric field, alongside a controllable magnetic field. The two fields are [[Orthogonality|orthogonal]] to each other, and also to the trajectory of the electron beam, which passes through the capacitor. This geometry is chosen to simplify the calculations, as we don't need to worry about angles of incidence. The [[Lorentz force]] is
...
- $B$ depends on the geometry and material of the instrument's electromagnet. If the details are known, it can be calculated analytically, or it can be measured with a [[Hall effect]] sensor.

The greatest obstacle to a good measurement in this experiment is guaranteeing an accurate confinement of the magnetic field to just the space between the capacitor plates. Given how difficult magnetic fields are to contain, it is likely to be the largest source of systematic errors in the measurement. It is also something that can't be fixed in the laboratory given that the geometry of $B$ is chosen by the manufacturer when engineering the tool, so a well-constructed Braun tube is essential for good results.