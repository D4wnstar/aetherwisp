...
### In statistics
The arithmetic mean commonly appears in statistics as a descriptive [[statistic]] of a [[sample]], both in the sense [[Random variable|random variables]] and data points. Despite it's simplicity, it benefits from remarkable properties that make it a commonly employed statistic.
...
In other words, the sample (arithmetic) mean is an unbiased [[estimator]] of the true mean. The [[variance]] follows suit:
...
This form goes back to the previous one for iid variables. The sample mean variance is therefore an estimator of the (average) population variance, albeit scaled by $1/N$. However, unlike the mean, it is not perfect. It is found that this value consistently diverges from the population variance, with the effect being more pronounced for small $N$. In other words, the sample mean variance is *biased*. Thankfully, the bias can be removed using the **Bessel correction**:
$$\boxed{\text{var}(\bar{X})=\frac{\bar{\sigma} ^{2}}{N-1}}$$
where we changed $N$ to $N-1$. This corrected sample mean variance is entirely unbiased.
...
In other words, as the number of random variables (in practice, data points) grows large, the sample mean becomes a progressively better estimator.